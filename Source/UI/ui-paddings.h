#pragma once

#include <JuceHeader.h>

#include <cstdint>

#include "UI/ui-roles.h"

//-----------------------------------------------------------------------------

// Themed paddings: enum token, "block/key" path in the theme file (blocks
// group all of a component's properties, CSS-class style), single default
// applied to all four sides.
// The theme file takes CSS shorthand: one value = all sides, two = top/bottom
// left/right, three = top left&right bottom, four = top right bottom left.
// Negative values grow the rect instead of shrinking it
#define PADDING_ROLES(X) \
	X(tooltip,			"tooltip/padding",		9.0f) \
	X(grid_mini,		"grid-mini/padding",	8.0f) \
	X(chip_history,		"chip-states/history-padding",		2.0f) \
	X(chip_filter_mode,	"chip-states/filter-mode-padding",	4.0f) \
	X(chip_waveform,	"chip-states/waveform-padding",		6.0f) \
	X(chip_control,		"chip-states/control-padding",		2.0f) \
	X(dialog_entry,		"dialog/entry-padding",				8.0f) \
	X(stil_box,			"stil-box/padding",					10.0f) \
	X(stil_box_gap,		"stil-box/item-gap",				5.0f) \
	X(stil_marker,		"stil-box/marker-padding",			6.0f) \
	X(stil_author,		"stil-box/author-padding",			8.0f) \
	X(stil_author_height, "stil-box/author-height",			50.0f) \
	X(stil_box_alpha,	"stil-box/alpha",					0.15f) \
	X(eq_curve_alpha,	"eq/curve-alpha",					0.85f) \
	X(eq_bars_alpha,	"eq/bars-alpha",					1.0f) \
	X(eq_dash_visibility,	"eq/dash-visibility",			1.0f) \
	X(eq_column_visibility,	"eq/column-visibility",			1.0f) \
	X(stil_title_gap,	"stil-box/title-icon-gap",			4.0f) \
	X(stil_link,		"stil-box/link-padding",			2.0f) \
	X(stil_list_icon,	"stil/list-icon-padding",			1.0f) \
	X(stil_title_icon,	"stil-box/title-icon-padding",		2.0f) \
	X(toast_padding,	"toast/padding",					18.0f) \
	X(toast_button_padding,	"toast/button-padding",			8.0f) \
	X(toast_progress,	"toast/progress-height",			4.0f) \
	X(keycap,			"keycap/padding",					7.0f) \
	X(keycap_height,	"keycap/height",					24.0f) \
	X(keycap_gap,		"keycap/gap",						4.0f) \
	X(shortcuts_row,	"shortcuts/row-height",				34.0f) \
	X(shortcuts_header,	"shortcuts/header-height",			60.0f) \
	X(focus_ring,		"focus-ring/padding",				3.0f)

namespace UI::paddings
{
	enum Role : int8_t
	{
		#define X(role, name, p) role,
		PADDING_ROLES ( X )
		#undef X
		count
	};

	// Per-side padding, CSS clockwise order
	struct Def
	{
		float	top;
		float	right;
		float	bottom;
		float	left;
	};

	// Resolves a role by its theme-file path ("stil-box/padding", ...);
	// count = no such role
	[[ nodiscard ]] inline Role fromName ( const juce::String& roleName )
	{
		static constexpr const char* names[] =
		{
			#define X(role, name, p) name,
			PADDING_ROLES ( X )
			#undef X
		};

		return Role ( UI::roleIndex ( names, int ( std::size ( names ) ), roleName ) );
	}
}
//-----------------------------------------------------------------------------
