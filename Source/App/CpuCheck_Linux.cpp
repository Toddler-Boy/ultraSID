...
__attribute__ (( constructor ( 101 ) )) static void checkCpuBaseline ()
{
	...
	const std::string	message = "ultraSID needs a CPU with SSE 4.2 (Intel Nehalem, AMD Bulldozer or newer).";

	std::fprintf ( stderr, "%s\n", message.c_str () );

	if ( std::system ( ( "zenity --error --title=ultraSID --text=\"" + message + "\" 2>/dev/null" ).c_str () ) != 0 )
